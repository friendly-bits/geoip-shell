...
Otherwise, geoip rules are applied to traffic going through all network interfaces, except the loopback interface. Besides that, when geoip-shell is configured in whitelist mode and you picked `n` in this question, additional firewall rules may be created which add LAN IP addresses or IP ranges to the whitelist in order to avoid blocking them (you can approve or configure this on the next step of the automated setup). This does not guarantee that your LAN IP ranges will not be blocked by another rule in another table, and in fact, if you prefer to block some of them then having them in whitelist will not matter. This is because while the 'drop' verdict is final, the 'accept' verdict is not.

### **'Automatically detected ipvX LAN IP ranges: ... [c]onfirm, c[h]ange, [s]kip or [a]bort?'**
...
Otherwise consider adding LAN IP ranges to the whitelist. You can either confirm the automatically detected IP ranges, or specify any combination of IPs and IP ranges on your LAN which you wish to allow connections from (or to, if using outbound geoblocking).

The automatic detection code should, in most cases, detect correct LAN IP ranges. However, it is up to you to verify that it's done its job correctly.

One way to do that is by typing in 'c' to confirm and once automated setup completes, verifying that you can still access the machine from LAN (note that if you have an active connection to that machine, for example through SSH, it will likely continue to work until disconnection even if autodetection of LAN IP ranges did not work out correctly).
Of course, this is risky in cases where you do not have physical access to the machine.

Another way to do that is by checking which IP address you need to access the machine from, and then verifying that said IP address is included in one of the automatically detected IP ranges. For example, if your other machine's IP is `192.168.1.5` and one of the detected IP ranges is `192.168.1.0/24` then you will want to check that `192.168.1.5` is included in IP range `192.168.1.0/24`. Provided you don't know how to make this calculation manually, you can use the `grepcidr` tool this way:
`echo "192.168.1.5" | grepcidr "192.168.1.0/24"`

The syntax to check in multiple IP ranges (note the double quotes):
`echo "[IP]" | grepcidr "[IP_range_1] [IP_range_2] ... [IP_range_N]"`
...
A third way to do that is by examining your network configuration (in your router) and making sure that the automatically detected IP ranges match those in the config.

If you find out that the IP ranges were detected incorrectly, you can type in 'h' and manually enter the correct IP ranges or IP addresses which you want to allow connections from.

### **'A[u]tomatically detect LAN IP ranges when updating IP lists or keep this config c[o]nstant?'**

As the above question, you will see this one if configuring geoip-shell in whitelist mode and you answered `n` to the question about WAN interfaces. You will not see this question if you specified custom IP ranges or ips in the previous question.

The rationale for this question is that network configuration may change, and if it does then previously correctly configured LAN IP ranges may become irrelevant.

If you type in 'a', each time geoip firewall rules are initialized or updated, LAN IP ranges will be re-detected.

If you type in 'c' then whatever IP ranges have been detected during the initial setup will be remembered forever (until you manually change the setting).
